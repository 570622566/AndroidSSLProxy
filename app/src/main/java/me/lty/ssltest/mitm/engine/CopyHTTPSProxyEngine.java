 ...
 * @version v1.0
 */
public class CopyHTTPSProxyEngine extends ProxyEngine {
    ...
    private static final String TAG = CopyHTTPSProxyEngine.class.getSimpleName();
    ...
    /**
     * When client A do handshake with server A. Sleep thread until handshake completed.
     */
    private boolean handshakeCompleted = false;

    public CopyHTTPSProxyEngine(Socket acceptSocket,
                                MITMSSLSocketFactory sslSocketFactory,
                                ProxyDataFilter requestFilter,
                                ProxyDataFilter responseFilter,
                                String localHost,
                                int localPort, LruCache<String,MITMSSLSocketFactory> ssfCache)
        ...
        m_proxySSLEngine = new InnerProxySSLEngine(sslSocketFactory, requestFilter, responseFilter,ssfCache);
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (httpsConnectMatcher.find()) {//then we have a proxy CONNECT message!
                ...
                final Socket sslProxySocket = new Socket(getConnectionDetails().getLocalHost(), localProxy.getLocalPort());
    ...
    private class InnerProxySSLEngine extends ProxyEngine {
        ...
        InnerProxySSLEngine(MITMSSLSocketFactory socketFactory,
            ...
            super(
                    ...
                    requestFilter,
                    responseFilter,
                    new ConnectionDetails(CopyHTTPSProxyEngine.this.getConnectionDetails()
                                                                   .getLocalHost(),
                                          0, "", -1, true
                    )
            );
